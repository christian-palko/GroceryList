...
class GroceryListCLI extends AnsiColor {
    ...
    while (keepAdding == true) {
      ...
      for (a <- add) {
        ...
        if (itemNamesAndTagsMap.contains(a.trim)) {
          println(s"${RED}${BOLD}* List already contains $a *${RESET}")
        } else {
          trimmedItem match {
            case "BACK" | "B" | "EXIT" | "QUIT" | "STOP" =>
              keepAdding = false
              println(s"${GREEN}* Cancel item add. *${RESET}")
            case _ =>
  ...
  def appendListWithImport() {
    ...
    try {
      ...
      val openFileAdd = openFile.toUpperCase.trim
        .split(",")
        .to(ArrayBuffer)
  ...
  def deleteItem() {
    ...
    delete match {
      case "NO" | "N" | "BACK" | "B" | "CANCEL" | "EXIT" =>
        println(s"${GREEN}* Cancelled item deletion. *${RESET}")
      case _ =>
  ...
  def clearAll() {
    ...
    clear match {
      case "NO" | "N" | "BACK" | "B" | "CANCEL" | "EXIT" =>
        println(s"${GREEN}* Cancelled list clearing. *${RESET}")
      case "YES" | "Y" | "CLEAR" => ItemDao.clearAllFromDB()
      case _ =>
        println(s"${YELLOW}* Cancelled list clearing. Invalid input. *${RESET}")
    }
  }
  ...
  def changeItem() {
    ...
    itemChangeFrom match {
      case "NO" | "N" | "BACK" | "B" | "CANCEL" | "EXIT" =>
        println(s"${GREEN}* Cancelled item change. *${RESET}")
      case _ =>
        if (itemNamesAndTagsMap.contains(itemChangeFrom)) {
          var itemChangeTo = readLine(
          ...
          itemChangeTo match {
            ...
            case _ =>
              if (itemNamesAndTagsMap.contains(itemChangeTo)) {
                println(
                  s"\n${YELLOW}❋ This item is already in your list. Cancelled item change. ❋${RESET}"
                )
              } else {
  ...
  def exitApp() {
    println(s"\n${BLINK}${GREEN}${BOLD}Goodbye!${RESET}\n")
